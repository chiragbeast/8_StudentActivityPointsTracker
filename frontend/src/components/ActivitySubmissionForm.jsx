...
export default function ActivitySubmissionForm() {
    ...
    return (
        <div className="submission-form-page">
            ...
            {/* Loading state */}
            {loadingDraft ? (
                <div style={{ padding: '3rem', textAlign: 'center', color: '#6b7280' }}>Loading submission data...</div>
            ) : (
                <>
                    {/* Header */}
                    <div className="form-page-header">
                        <div>
                            <h1 className="form-page-title">{isEditing ? 'Edit Submission' : 'Activity Submission'}</h1>
                            <p className="form-page-subtitle">{isEditing ? 'Update your activity details and upload documents.' : 'Submit your activity details for point verification.'}</p>
                        </div>
                        <button type="button" className="guidelines-link" onClick={() => setShowGuidelines(true)}>
                    ...
                    <form className="submission-form" onSubmit={handleSubmit}>
                        ...
                        />
                        <p className="field-tip">Tip: Refer to the handbook for standard point values per activity level.</p>
                        ...
                        {existingDocs.length > 0 && (
                            <div className="file-list" style={{ marginBottom: '1rem' }}>
                                <p className="field-label" style={{ marginBottom: '0.5rem' }}>Uploaded Documents</p>
                                {existingDocs.map((doc) => (
                                    <div className="file-item" key={doc._id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" style={{ flex: 1, color: '#4C9AFF', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        ...
                        {files.length > 0 && (
                            <div className="file-list">
                                <p className="field-label" style={{ marginBottom: '0.5rem' }}>New Files to Upload</p>
                                {files.map((f, i) => (
                                    <div className="file-item" key={i}>
                                        <span>{f.name}</span>
                                        <span style={{ fontSize: '0.75rem', color: '#9ca3af', marginLeft: 'auto', marginRight: '0.5rem' }}>
                        ...
                        <div className="form-actions">
                            <button type="submit" className="submit-btn" disabled={submitting}>
                                {submitting ? 'Uploading...' : (isEditing ? 'UPDATE & SUBMIT FOR REVIEW' : 'SUBMIT FOR REVIEW')}
                            </button>
                            <div className="form-actions-secondary">
                                <button type="button" className="draft-btn" onClick={handleSaveDraft} disabled={submitting}>
